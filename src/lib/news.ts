export async function getNews(category: 'brasil' | 'internacional' | 'mercado' = 'brasil') {
  try {
    ...
    if (!apiKey) {
      throw new Error("NEWS_APIKEY is not defined");
    }
    let params;
    switch (category) {
      case 'internacional':
    ...
    // Corrected: Semicolon moved outside the backticks and cache parameter named
    const url = `https://newsapi.ai/api/v1/article/getArticles?${params.toString()}&cache=${new Date().getTime()}`;
    ...
    const articles = data.articles.results
      ...
      .map((article: any) => {
        if (article.image && !article.image.startsWith('http')) {
          // Corrected: Used backticks (`) for string assignment
